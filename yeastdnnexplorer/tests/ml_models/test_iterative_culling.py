...
from iterative_culling import (
    add_main_effects,
    create_and_fit_combined_filtered_models,
    create_formulas_with_max_lrb,
    create_max_LRB_columns,
    find_common_features_across_tfs,
    iterative_model_selection,
    process_tf_data_combined,
)
...
def test_find_common_features_across_tfs(tf_list, sample_data, max_formulas):
    common_features = find_common_features_across_tfs(
        tf_list, sample_data, max_formulas
    )
...
def test_process_tf_data_combined(tf_list, sample_data, max_formulas):
    results_df = process_tf_data_combined(
        "TF1", sample_data, max_formulas, filter_top_10=False
    )
