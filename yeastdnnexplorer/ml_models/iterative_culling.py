...
def create_formulas_with_max_lrb(tf_list):
    ...
    for tf in tf_list:
        ...
        interaction_terms = ' + '.join([f'LRB_{tf}:LRB_{other_tf}' for other_tf in tf_list if other_tf != tf])
...
def process_tf_data_combined(tf_name, data, formulas, filter_top_10=False, quantile_threshold=0.9, n_splits=4, random_state=42):
    ...
    # Create a patsy dmatrix - note that we don't use an intercept as we will specify that in the LassoCV model
    y, X = dmatrices(formula[0]+ " - 1", data, return_type='dataframe')

    # Make splits for CV
    skf = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=random_state)
    splits = list(skf.split(cv_data[f'target_locus_tag'].values, cv_data["classification"]))
...
def find_common_features_across_tfs(tfs, data, formulas, quantile_threshold=0.9):
    """
    Find common features across TFs by processing both the full dataset and the top 10% data.
    ...
    Returns:
    dict: A dictionary where each key is a TF and the value is a list of common features.
    """
    ...
    for tf in tfs:
        ...
        df_top10 = process_tf_data_combined(tf, data, formulas, filter_top_10=True, quantile_threshold=quantile_threshold)
    ...
    # Return as a dictionary where the response TF is the key, and the value is a list of the surviving features
...
def filter_significant_features(y, X, threshold=0.001):
        ...
        # helper method for create_and_fit_combined_filtered_models that does the culling process using a given p-value threshold
        model = sm.OLS(y, X).fit()
        return [feature for feature, pval in model.pvalues.items() if pval < threshold and feature != 'Intercept'], model

def create_and_fit_combined_filtered_models(common_features_dict, data, full_data_iterations=1, top10_data_iterations=1):
    """
    Combine filtering phases on the full dataset and the top 10% by LRB_{tf} data.

    Parameters:
    common_features_dict (dict): Dictionary with TFs as keys and list of common features as values.
    data (DataFrame): The original DataFrame.
    full_data_iterations (int, optional): Number of filtering iterations on the full dataset.
    top10_data_iterations (int, optional): Number of filtering iterations on the top 10% data.

    Returns:
    dict: A dictionary where each key is a TF and the value is the final fitted model or None if no model survived.
    """
    final_fitted_models = {}
    ...
    for tf, features in common_features_dict.items():
        ...
        if survived_all_iterations:
            ...
            y_final, X_final = dmatrices(final_formula, top_10_data, return_type='dataframe')
...
def get_existing_formulas_and_tfs(model_dict):
    """
    Extract formulas from existing models in the dictionary and collect TF names from LRR_{tf}.

    Parameters:
    model_dict (dict): Dictionary where keys are TFs and values are statsmodels models or None.
    ...
    for tf, model in model_dict.items():
        if model is not None:  
            ...
            predictors = [pred for pred in model.model.exog_names if pred != 'Intercept']
...
def get_cross_validation_folds_from_dataframe(df, n_splits, tf):
    ...
    return [(X[train_idx].tolist(), X[test_idx].tolist()) for train_idx, test_idx in skf.split(X, df['classification'])]
...
def classify_genes(df, tf):
    ...
    classification_df['binding_rank'] = classification_df[binding_col].rank(method='min', ascending=False)
    classification_df['perturbation_rank'] = classification_df[perturbation_col].rank(method='min', ascending=False)

    # Binning ranks and combining classifications
    bins = [0, 8, 64, 512, np.inf]
    classification_df['binding_bin'] = pd.cut(classification_df['binding_rank'], bins=bins, labels=[1, 2, 3, 4]).astype(int)
    classification_df['perturbation_bin'] = pd.cut(classification_df['perturbation_rank'], bins=bins, labels=[1, 2, 3, 4]).astype(int)
    classification_df['classification'] = (classification_df['binding_bin'] - 1) * 4 + classification_df['perturbation_bin']

    return classification_df.sort_values('classification')

def create_max_LRB_columns(df):
    """
    For each column of the form 'LRB_{tf}' in the DataFrame, create a new column 'max_LRB_{tf}'
    that contains the maximum value from all other 'LRB_{other_tf}' columns for each row.
...
def custom_wrapper_cross_validation(df, n_splits, tf, current_formula):
    ...
    Returns:
    - results_df: DataFrame containing average R-squared values, formula type, and formula.
    """
    formulas_to_test = {'Current': current_formula}
    predictors = current_formula.split(" ~ ")[1]
    
    # Extract and modify interaction terms
    interaction_terms = [term for term in predictors.split(" + ") if term.startswith(f"LRB_{tf}:")]
    for term in interaction_terms:
        main_effect = term.split(":")[1]
        formulas_to_test[f"Replace_{main_effect}"] = current_formula.replace(term, main_effect)
    ...
    for formula_name, formula in formulas_to_test.items():
        r2_list = [evaluate_model(formula, train, test, df, tf) for train, test in folds]
        avg_r2 = sum(r2_list) / len(r2_list)
        results.append({'TF': tf, 'Formula_Type': formula_name, 'Avg_R_squared': avg_r2, 'Formula': formula})

    return pd.DataFrame(results)

def iterative_model_selection(df, n_splits, tf, final_formulas):
    """
    Iteratively refines the model by evaluating each interaction term and returns a DataFrame
    with the response TF, each feature in the final formula, and their coefficients sorted by
    the magnitude of the coefficient (excluding the intercept).

    Parameters:
    - df: pandas DataFrame containing the data for the models.
    - n_splits: Number of folds for cross-validation.
    - tf: The transcription factor of interest.
    - final_formulas: A list of final formulas output by complex models to get the multivariate formula.

    Returns:
    - result_df: DataFrame containing the response TF, each feature in the final formula,
                 and the coefficient of that feature, sorted by coefficient magnitude.
    """
    # Find the original multivariate formula from the list
    multivariate_formula = next((s for s in final_formulas if s.startswith(f'LRR_{tf}')), None)
    if multivariate_formula is None:
        raise ValueError(f"No multivariate formula found for {tf}.")

    logger.info(f"Starting iterative model selection with original formula: {multivariate_formula}")
    ...
    initial_results_df = custom_wrapper_cross_validation(df, n_splits, tf, multivariate_formula)
    original_avg_r2 = initial_results_df.loc[initial_results_df['Formula_Type'] == 'Current', 'Avg_R_squared'].mean()

    # Extract interaction terms from the original formula
    interaction_terms = [term for term in terms if ":" in term]

    # We make a dict. of necessary modifications so that we can keep track of everything that needs to be updated at the end
    modifications = {}
    ...
    for interaction_term in interaction_terms:
        ...
        iteration_results_df = custom_wrapper_cross_validation(df, n_splits, tf, multivariate_formula)

        # Filter for the specific formulas from the CV output based on the current interaction term
        relevant_formulas = iteration_results_df[
            iteration_results_df['Formula_Type'].isin([f"Current", f"Replace_{main_effect}", f"Add_{main_effect}"])
        ]
        ...
        if best_model['Avg_R_squared'] > best_avg_r2:
            ...
            logger.info(f"Selected modification for '{interaction_term}': {best_model['Formula_Type']} with Avg R-squared = {best_avg_r2}")
    ...
    for interaction_term, (action, main_effect) in modifications.items():
        if action == 'replace':
            final_terms = [term if term != interaction_term else main_effect for term in final_terms]
        elif action == 'add':
    ...
    model = sm.OLS(y, X).fit()
    coefficients = model.params.drop('Intercept', errors='ignore')  # Exclude the intercept
...
def add_main_effects(final_models, data):
    ...
    for tf in final_tf_list:
        ...
        combined_results_final_df = pd.concat([combined_results_final_df, results_df], ignore_index=True)
